...
#             fig.colorbar(im, ax=axes[3:], fraction=0.015, pad=0.06)
...
def least_squares_posterior_estimation(model, input_data, true_data, model_type, learning_rate, num_iterations=500):
    ...
    for iteration in range(num_iterations):
        ...
        plot_single_abs(input_data.detach().cpu()[0, 0], f'GCS_partial/vec={num_vec}/iter_{model_type}_{iteration}', cmap='Blues')
        ...
        for batch in [0, 10, 20, 30, 40, 50]:
            plot_single(output[batch][-1].detach().cpu().numpy(), f"GCS_partial/vec={num_vec}/lr={learning_rate}/S/{model_type}_{batch}_{iteration}")
            plot_single(true_data[batch][-1].detach().cpu().numpy(), f"GCS_partial/vec={num_vec}/lr={learning_rate}/S/True_{batch}")
        if model_type == "MSE":
        ...
        print(input_data.detach()[0][0], input_data.shape)
        posterior_set.append(input_data.detach())
        print("update", posterior_set[iteration][0][0])
