...
org_set_y = set_y
plot_multiple_abs(org_set_y[0], f'GCS_partial_constrained/S_org', cmap='Blues')
# three column.
first_col_idx = 0
...
for s_idx in range(1, 1001):
    with h5py.File(f'../FNO-NF.jl/scripts/num_obs_2/FIM_eigvec_sample_{s_idx}_nobs_2.jld2', 'r') as f1:
        S = f1['single_stored_object'][:]
        set_eig.append(torch.tensor(S).reshape(8, 64, 64))

for s_idx in range(1, 1001):
    with h5py.File(f'../FNO-NF.jl/scripts/num_obs_2/FIM_vjp_sample_{s_idx}_nobs_2.jld2', 'r') as f1:
        S = f1['single_stored_object'][:]
        set_vjp.append(torch.tensor(S).reshape(8, 64, 64))

train_vjp = torch.stack(set_vjp[900:])
print("vjp norm", torch.norm(train_vjp))
train_vjp = train_vjp / torch.norm(torch.stack(set_vjp))

set_eig = torch.stack(set_eig[900:])
print("set_eig", set_eig.shape)

'''constrained_same eig'''
'''train_vjp = train_vjp.unsqueeze(1)  # Now tensor is [25, 1, 64, 64]
train_vjp = train_vjp.repeat(1, 8, 1, 1)  # Now tensor is [25, 8, 64, 64]
set_eig = set_eig[:, 0] # ([100, 3, 64, 64]) -> ([100, 1, 64, 64])
...
# set_y = set_y * mask
plot_multiple_abs(set_y[0], f'GCS_partial_constrained/S_masked3', cmap='Blues')
...
def least_squares_posterior_estimation(model, input_data, true_data, model_type, learning_rate, num_iterations=500):
    ...
    for iteration in range(num_iterations):
        ...
        plot_single_abs(input_data.detach().cpu()[0, 0], f'GCS_partial_constrained/iter_{model_type}_{iteration}', cmap='Blues')
        ...
        plt.grid(True)
        plt.savefig(f'GCS_partial_constrained/{model_type}_{num_col}/loss_plot_{learning_rate}_{num_epoch}.png')

        # if iteration % 50 == 0:
        print(f"Iteration {iteration}, Loss: {loss.item(), jac_misfit}")
        posterior_set.append(input_data.detach())

    # Return the optimized input data (permeability K)
    return posterior_set, losses, mse_losses  # Detach from the computational graph
...
for i in range(num_batch):
    ...
    print(zero_X.shape)
    #######
    posterior_estimate_mse, mse_losses, mse_mse_losses = least_squares_posterior_estimation(MSE_model, zero_X, Y_true, "MSE", learning_rate, num_iterations=num_epoch)
    posterior_estimate_jac, jac_losses, jac_mse_losses = least_squares_posterior_estimation(JAC_model, zero_X, Y_true, "JAC", learning_rate, num_iterations=num_epoch)
    ...
    plt.legend(fontsize=15)
    plt.grid(True)
    plt.savefig(f'GCS_partial_constrained/both_{num_col}/loss_plot_{learning_rate}_{num_epoch}.png')
    ...
    for t in range(0, num_epoch, 50):
        ...
        for b in range(batch_size): # for every test sample
            ...
            path = f'GCS_partial_constrained/both_{num_col}_200/posterior_{t}_{i}_{b}'
    ...
    for b in range(batch_size): # for every test sample
        ...
        ssim_mse = ssim(posterior_estimate_mse[t][b][-1].detach().cpu().unsqueeze(dim=0).unsqueeze(0), X[b][-1].detach().cpu().unsqueeze(dim=0).unsqueeze(dim=0))
        ssim_jac = ssim(posterior_estimate_jac[t][b][-1].detach().cpu().unsqueeze(dim=0).unsqueeze(0), X[b][-1].detach().cpu().unsqueeze(dim=0).unsqueeze(dim=0))
        path = f'GCS_partial_constrained/both_{num_col}_{learning_rate}/posterior_{t}_{i}_{b}'
        ssim_all_mse += ssim_mse
        ssim_all_jac += ssim_jac
        print("shape", X[b][-1].shape, zero_X[b][-1].shape, posterior_estimate_mse[t][b][-1].shape)
        
        plot_diff_with_shared_colorbar_all([X[b][-1].detach().cpu(), zero_X[b][-1].detach().cpu(), posterior_estimate_mse[t][b][-1].detach().cpu(), posterior_estimate_jac[t][b][-1].detach().cpu(), abs_diff_mse, abs_diff_jac], t, ssim_mse, ssim_jac, path, cmap='magma')
    print("PBI SSIM Full:", ssim_all_jac, "PBI losses", jac_mse_losses[-1])
    print("MSE SSIM Full:", ssim_all_mse, "MSE losses:", mse_mse_losses[-1])
    

    # PBI SSIM Full: tensor(55.7771) 0.5578
    # MSE SSIM Full: tensor(53.6362) 0.5363