...
while t < tEnd:
    ...
    vx = torch.tensor(vx, requires_grad=True)
    vy = torch.tensor(vy, requires_grad=True)
    P = torch.tensor(P, requires_grad=True)

    # Compute the solution
    solution = torch.stack([rho, vx, vy, P], dim=-1)

    # Compute the Jacobian
    jacobian = torch.autograd.functional.jacobian(solution, torch.stack([rho, vx, vy, P], dim=1))

    # Compute the vector Jacobian product
    input_vec = torch.randn_like(solution)
    vector_jacobian_product = torch.autograd.functional.vjacobian(solution, torch.stack([rho, vx, vy, P], dim=1), v=input_vec)
