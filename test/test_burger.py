...
def solve_burger_equation(u, v, nu=0.01, nx=41, ny=41, num_iterations=500, dt=0.001):
    dx = dy = 2.0 / (nx - 1)
    ...
    for n in range(num_iterations):
        ...
        # Applying the finite difference method for both u and v
        u[1:-1, 1:-1] = (u_old[1:-1, 1:-1] 
                         - dt * u_old[1:-1, 1:-1] / dx * (u_old[1:-1, 1:-1] - u_old[:-2, 1:-1])
                         - dt * v_old[1:-1, 1:-1] / dy * (u_old[1:-1, 1:-1] - u_old[1:-1, :-2])
                         + nu * dt / dx**2 * (u_old[2:, 1:-1] - 2 * u_old[1:-1, 1:-1] + u_old[:-2, 1:-1])
                         + nu * dt / dy**2 * (u_old[1:-1, 2:] - 2 * u_old[1:-1, 1:-1] + u_old[1:-1, :-2]))

        v[1:-1, 1:-1] = (v_old[1:-1, 1:-1]
                         - dt * u_old[1:-1, 1:-1] / dx * (v_old[1:-1, 1:-1] - v_old[:-2, 1:-1])
                         - dt * v_old[1:-1, 1:-1] / dy * (v_old[1:-1, 1:-1] - v_old[1:-1, :-2])
                         + nu * dt / dx**2 * (v_old[2:, 1:-1] - 2 * v_old[1:-1, 1:-1] + v_old[:-2, 1:-1])
                         + nu * dt / dy**2 * (v_old[1:-1, 2:] - 2 * v_old[1:-1, 1:-1] + v_old[1:-1, :-2]))
...
def generate_dataset(num_samples, nx=50, ny=50, pattern='sinusoidal'):
    ...
    for s in range(num_samples):
        ...
        # constant k
        # Initial conditions: Cuboidic Wave Profile
        u = torch.ones((ny, nx))
        v = torch.ones((ny, nx))    
        u[int(0.75 / (2 / (ny - 1))):int(1.25 / (2 / (ny - 1)) + 1), int(0.75 / (2 / (nx - 1))):int(1.25 / (2 / (nx - 1)) + 1)] = 3
        v[int(0.75 / (2 / (ny - 1))):int(1.25 / (2 / (ny - 1)) + 1), int(0.75 / (2 / (nx - 1))):int(1.25 / (2 / (nx - 1)) + 1)] = 3

        # k = torch.ones(nx, ny)
        # Generate q with the selected pattern
        # q = create_q_function(nx, ny, noise_level=0.1, pattern=pattern)
        # Solve the burger equation
        T = solve_burger_equation(u, v, nx=nx, ny=ny)
        
        input.append(u)
        output.append(T)
        
        if s == 0:
            plot_path = f"../plot/Burger_plot/Burger_q_{pattern}.png"
            plot_data(u, v, T[0], plot_path)
        elif s == 1:
            plot_path = f"../plot/Burger_plot/Burger_q_{pattern}(1).png"
            plot_data(u, v, T[0], plot_path)
...
def compute_fim_for_2d_burger(solve_burger_equation, q, T_data, noise_std, nx=50, ny=50):
    ...
    T_pred = solve_burger_equation(k, q, nx, ny) + noise.cuda()
...
def plot_results(k, T_true, T_pred, path):
    ...
    axes[0].set_title(r"Burger Source $q$")
...
def plot_data(k, q, T, path):
    ...
    axes[1].set_title(r"Burger Source $q$")
    fig.colorbar(im1, ax=axes[1], fraction=0.045, pad=0.06)
...
def main(logger, args, loss_type, dataloader, test_dataloader, vec):
    ...
    if args.loss_type == "JAC":
        ...
        if os.path.exists(csv_filename):
            ...
            k = torch.ones(nx, ny)
            f = lambda x: solve_burger_equation(k.cuda(), x, nx, ny)
    ...
    for epoch in range(args.num_epoch):
        ...
        if epoch % 50 == 0:
            torch.save(model.state_dict(), f"../test_result/Checkpoint/FNO_Burger_{loss_type}_{args.nx}_{args.num_train}_{epoch}.pth")
        if full_test_loss < lowest_loss:
            print("saved lowest loss model")
            lowest_loss = full_test_loss
            torch.save(model.state_dict(), f"../test_result/best_model_FNO_Burger_{loss_type}.pth")
            # Save plot
            k, T = next(iter(test_dataloader))
            k, T = k.unsqueeze(dim=1).to(device).float(), T.to(device).float()
            with torch.no_grad():
                T_pred = model(k)
            plot_path = f"../plot/Burger_plot/FNO_Burger_lowest_{loss_type}.png"
    ...
    # Save the model
    torch.save(model.state_dict(), f"../test_result/best_model_FNO_Burger_full epoch_{loss_type}.pth")
    # Save the elapsed times
    with open(f'../test_result/Time/FNO_BURGER_{args.loss_type}_{args.nx}_{args.num_train}.csv', 'w', newline='') as csvfile:
    ...
    with torch.no_grad():
        T_pred = model(k)
    plot_path = f"../plot/Burger_plot/FNO_Burger_{loss_type}.png"
    plot_results(k[0], T[0], T_pred[0], plot_path)
    ...
    path = f"../plot/Loss/FNO_Burger_{loss_type}.png"
...
if __name__ == "__main__":
    ...
    out_file = os.path.join("../test_result/", f"FNO_Burger_norm_{start_time}.txt")
    ...
    if os.path.exists(trainx_file):
        ...
        train_x = BurgerDataset(load_dataset_from_csv(trainx_file, args.nx, args.ny))
        train_y = BurgerDataset(load_dataset_from_csv(trainy_file, args.nx, args.ny))
        test_x = BurgerDataset(load_dataset_from_csv(testx_file, args.nx, args.ny))
        test_y = BurgerDataset(load_dataset_from_csv(testy_file, args.nx, args.ny))
    else:
        print("Creating Dataset")
        input, output = generate_dataset(args.num_train + args.num_test, args.nx, args.ny)
        train_x = BurgerDataset(input[:args.num_train])
        train_y = BurgerDataset(output[:args.num_train])
        test_x = BurgerDataset(input[args.num_train:])
        test_y = BurgerDataset(output[args.num_train:])
    ...
    if args.loss_type == "JAC":
        ...
        fim = compute_fim_for_2d_burger(solve_burger_equation, train_x[0].cuda(), train_y[0].cuda(), noise_std, nx, ny).detach().cpu()
        # Compute FIM
        for s in range(args.num_sample - 1):
            print(s)
            # k = torch.exp(torch.randn(nx, ny)).cuda()  # Log-normal distribution for k
            fim += compute_fim_for_2d_burger(solve_burger_equation, train_x[s], train_y[s], noise_std, nx, ny).detach().cpu()
        fim /= args.num_sample
