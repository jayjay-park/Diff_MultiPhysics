...
def solve_darcy(K, f, nx=50, ny=50, num_iterations=1000):
    ...
    for num in range(num_iterations):
        ...
        h_old = h.clone()
        h[1:-1, 1:-1] = 0.25 * (
            K[1:-1, 1:-1] * (h_old[2:, 1:-1] + h_old[:-2, 1:-1] + h_old[1:-1, 2:] + h_old[1:-1, :-2])
            - dx * dy * f[1:-1, 1:-1]
        ) / K[1:-1, 1:-1]
...
def compute_fim_for_2d_heat(eq, k, q, T_data, noise_std, nx=50, ny=50):
    ...
    # Solve heat equation
    T_pred = eq(k, q.cuda(), nx, ny) + noise.cuda()
...
def main(logger, args, loss_type, dataloader, test_dataloader, vec):
    ...
    if args.loss_type == "JAC":
        ...
        if os.path.exists(csv_filename):
            ...
            f = lambda x: solve_darcy(x, q.cuda())
...
if __name__ == "__main__":
    ...
    T_data = solve_darcy(k, q.cuda(), nx, ny)  # This is your ground truth data
    noise_std = 0.01  # Adjust as needed

    fim = compute_fim_for_2d_heat(solve_darcy, k, q, T_data, noise_std, nx, ny).detach().cpu()
