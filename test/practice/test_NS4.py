...
def simulate_NS(N=400, tEnd=1.0, dt=0.001, nu=0.001, plotRealTime=True):
    ...
    def compute_velocity_from_vorticity(wz_input, kx, ky, kSq_inv):
        psi = poisson_solve(wz_input, kSq_inv)  # Stream function from vorticity
        vx, vy = grad(psi, kx, ky)
        vy = -vy  # Correct the direction for vx, vy
        return vx, vy

    def evolve_vorticity(wz_input, kx, ky, dealias, dt, nu, kSq, kSq_inv, Nt):
        # Convert vorticity to initial velocities
        ...
        # Convert final velocities back to vorticity
        wz_output = curl(vx, vy, kx, ky)
        
        return wz_output
    
    def main_function(wz_input, kx, ky, dealias, dt, nu, kSq, kSq_inv, Nt):
        wz_output = evolve_vorticity(wz_input, kx, ky, dealias, dt, nu, kSq, kSq_inv, Nt)
        return wz_output
    ...
    # Wrap input vorticity in a tensor with requires_grad=True
    wz_input = torch.tensor(wz_input, requires_grad=True)

    # Compute the output vorticity
    wz_output = main_function(wz_input, kx.cuda(), ky.cuda(), dealias.cuda(), dt, nu, kSq.cuda(), kSq_inv.cuda(), Nt)

    # Compute the Jacobian
    jacobian = torch.autograd.functional.jacobian(
        lambda wz_input: main_function(wz_input, kx, ky, dealias, dt, nu, kSq, kSq_inv, Nt),
        wz_input
    )
