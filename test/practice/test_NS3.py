...
def simulate_NS(N=400, tEnd=1.0, dt=0.001, nu=0.001, plotRealTime=True):
    ...
    def main_loop(vx, vy, kx, ky, dealias, dt, nu, kSq, kSq_inv, Nt):
        t = 0.0
        wz_output_list = []
        ...
        for i in range(Nt):
            ...
            wz_output_list.append(wz_output)
            
            # Update time
            t += dt
        
        return vx, vy, wz_output_list
    ...
    vx, vy, wz_output_list = main_loop(vx, vy, kx, ky, dealias, dt, nu, kSq, kSq_inv, Nt)
    ...
    im4 = axes[1, 1].imshow(wz_output_list[-1], cmap='RdBu')
    ...
    return vx, vy, wz_input, wz_output_list[-1]
