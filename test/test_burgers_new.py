...
def save_plot(vx_init, vy_init, vx_final, vy_final, filename):
    ...
    # Plot vx field
    im1 = axes[0, 0].imshow(vx_init.cpu().numpy(), cmap='viridis')#, origin='lower', extent=[0, 1, 0, 1])
    ...
    # Plot vy field
    im2 = axes[0, 1].imshow(vy_init.cpu().numpy(), cmap='viridis')#, origin='lower', extent=[0, 1, 0, 1])
    ...
    im3 = axes[1, 0].imshow(vx_final.cpu().numpy(), cmap='viridis')#, origin='lower', extent=[0, 1, 0, 1])
    ...
    im4 = axes[1, 1].imshow(vy_final.cpu().numpy(), cmap='viridis')#, origin='lower', extent=[0, 1, 0, 1])
...
def generate_dataset(simulator, num_samples=500, n_steps=200):
    ...
    for s in range(int(num_samples)):
        ...
        vx, vy = simulator.initialize_conditions()

        # Simulate for n_steps
        with torch.no_grad():
            vx_final, vy_final = vx.clone(), vy.clone()
            vx_final, vy_final, wz, seq_val = simulate(simulator, vx_final, vy_final, n_steps)
        ...
        if s < 10:
            ...
            save_plot(vx, vy, vx_final, vy_final, f'Burgers_dataset_{s}.png')
...
def compute_fim_Burgers(simulator, q, T_data, noise_std, nx=50, ny=50):
    q = q.requires_grad_()
    vx = q[0]
    vy = q[1]
    ...
    T_pred = torch.stack([out_vx, out_vy]) + noise.cuda()
    ll = log_likelihood(T_data, T_pred, noise_std)
    flat_Jacobian = torch.autograd.grad(inputs=q, outputs=ll, create_graph=True)[0].flatten()
...
def main(logger, args, loss_type, dataloader, test_dataloader, vec, simulator):
    ...
    if args.loss_type == "JAC":
        csv_filename = f'data/true_j_BG_{nx}_{args.num_train}.csv'
    ...
    for epoch in range(args.num_epoch):
        ...
        if epoch % 50 == 0:
            torch.save(model.state_dict(), f"test_result/Checkpoint/FNO_BG_{loss_type}_{args.nx}_{args.num_train}_{epoch}.pth")
        if full_test_loss < lowest_loss:
            print("saved lowest loss model")
            lowest_loss = full_test_loss
            torch.save(model.state_dict(), f"test_result/best_model_FNO_BG_{loss_type}.pth")
            # Save plot
            ...
            with torch.no_grad():
                Y_pred = model(X_test)
            plot_path = f"plot/BG_plot/FNO_BG_lowest_{loss_type}.png"
    ...
    # Save the model
    torch.save(model.state_dict(), f"test_result/best_model_FNO_BG_full epoch_{loss_type}.pth")
    # Save the elapsed times
    with open(f'test_result/Time/FNO_BG_{args.loss_type}_{args.nx}_{args.num_train}.csv', 'w', newline='') as csvfile:
    ...
    plot_path = f"plot/BG_plot/FNO_BG_{loss_type}.png"
    ...
    path = f"plot/Loss/FNO_BG_{loss_type}.png"
...
if __name__ == "__main__":
    ...
    out_file = os.path.join("test_result/", f"Burgers_{start_time}.txt")
    ...
    # Instantiate the BurgersSimulator
    simulator = BurgersSimulator(L=L, M=M, Nx=Nx, Ny=Ny, Nt=Nt, nu_mean=args.nu).to(device)
    datafile = f'data/Burgers_{args.nx}_{args.ny}_{args.num_train}_{args.num_test}.npy'
    ...
    for num in range(int(num_samples)):
        ...
        one_series_vx = torch.tensor(one_series['vx'], dtype=torch.float32)
    ...
    # print(len(inputs))
    # print(num_samples, args.num_train)
    # print(f'Inputs shape: {len(inputs_train)}')
    
    if args.loss_type == "JAC":
        noise_std = 0.01
        fim = compute_fim_Burgers(simulator, inputs_train[0].cuda(), outputs_train[0].cuda(), noise_std, nx=50, ny=50)
        for s in range(args.num_sample - 1):
            fim += compute_fim_Burgers(simulator, inputs_train[s].cuda(), outputs_train[s].cuda(), noise_std, nx=50, ny=50)
        fim /= args.num_sample

        eigenvalues, eigenvec = torch.linalg.eigh(fim)
        sorted_indices = torch.argsort(eigenvalues, descending=True)
        largest_eigenvector = eigenvec[:, sorted_indices[0]].reshape(2, args.nx, args.ny)
    else:
