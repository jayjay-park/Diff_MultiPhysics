...
def compute_fim_NS(simulator, input, prev_state, noise_std, nx, ny, forcing, time_step, Re, input_index, s, num_observations):
    '''
    ...
    for j in range(num_observations):
        ...
        T_data = simulator(prev_state.cuda().requires_grad_(), f=forcing, T=time_step, Re=Re)
    ...
    if args.loss_type == "JAC":
        ...
        if os.path.exists(csv_filename):
            ...
            for s in range(args.num_train):
                ...
                fim = compute_fim_NS(ns_solver, input_param, train_x_raw[s], noise_std, nx, ny, forcing, args.time_step, Re, init_index, s, num_observations=args.num_obs).detach().cpu()
                # Analyze the FIM
