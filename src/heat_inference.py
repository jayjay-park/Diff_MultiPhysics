...
def model(observed=None):
    # Prior for log(k)
    log_k = pyro.sample("log_k", dist.Normal(torch.zeros(nx, ny, device=device), 
                                             torch.ones(nx, ny, device=device)).to_event(2))
    k = torch.exp(log_k)
...
im2 = axes[1, 0].imshow(true_T.cpu().numpy(), cmap='viridis')
axes[1, 0].set_title("True Temperature (T)")
fig.colorbar(im2, ax=axes[1, 0])

im3 = axes[1, 1].imshow(observed_T.cpu().numpy(), cmap='viridis')
